...
node dist/cli.js cover {markdown_path} --client {client} --style {follow article tone|editorial|blueprint|notion|warm|watercolor|scientific|lofi-doodle|multi-panel-manga|notebook-sketch|claymation} --provider {gemini|openai|doubao|qwen}
node dist/cli.js illustrate {markdown_path} --client {client} --style {follow article tone|editorial|blueprint|notion|warm|watercolor|scientific|lofi-doodle|multi-panel-manga|notebook-sketch|claymation} --density {minimal|balanced|per-section|custom} --provider {gemini|openai|doubao|qwen}
...
`illustrate` inserts generated inline images under selected `##` / `###` sections and writes a bundled article package under `output/{client}/{date}-{title-slug}/` by default.
`publish` writes the draft metadata back to `clients/{client}/history.yaml` after draft creation succeeds. It infers `{client}` from the article path under `output/{client}/...`; use optional `--client` only when the markdown path does not follow the default bundle layout.
...
- `cover + inline images`
  - `node dist/cli.js cover ... --style {style}`
  - `node dist/cli.js illustrate ... --style {style} --density {density}`
- `cover only`
  - `node dist/cli.js cover ... --style {style}`
- `inline only`
  - `node dist/cli.js illustrate ... --style {style} --density {density}`
...
- `--density` applies only to inline images
- use `--max-images <n>` only when the user explicitly chose `custom` and provided a number
