 ...
 *
 * Uses the dynamic theme engine + cheerio for robust HTML manipulation.
...
export class WeChatConverter {
  ...
  convert(markdownText: string): ConvertResult {
    ...
    markdownText = stripPrimaryTitle(markdownText);
  ...
  private applyInlineStyles($: cheerio.CheerioAPI): void {
    ...
    $('li').each((_, elem) => {
      ...
      $(elem).children('p').each((_, p) => {
        ...
        const listText = s.liText || '';
        const nextStyle = [compact, listText, pStyle].filter(Boolean).join('; ');
        $(p).attr('style', nextStyle);
      });
    });
